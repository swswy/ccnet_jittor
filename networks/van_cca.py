...
class VAN(nn.Module):
    def __init__(self, img_size=512, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
...
class VAN_CCNet(nn.Module):
    def __init__(self, num_classes, recurrence=2, pretrained=False):
        self.backbone = van_base(pretrained=pretrained)
        self.recurrence = recurrence
        self.head = RCCAModule(512, 256, num_classes)
        self.conv320_512 = nn.Conv2d(320,512,1)
        # self.conv128_512 = nn.Conv2d(128,512,1)
    ...
    def execute(self,x):
        output_s4,output_s8,output_s16,output_s32 = self.backbone(x)

        # output_s8 = self.conv128_512(output_s8)
        output_s16 = self.conv320_512(output_s16)
        output_s32 = self.upsample2x(output_s32)
        output_s16 = output_s16 + output_s32
        output_s16 = self.upsample16x(output_s16)
        # output_s8 = output_s8 + output_s16
        # output_s8 = self.upsample8x(output_s8)
        final_output = self.head(output_s16, self.recurrence)
        return final_output

def CCNet_Model(num_classes, recurrence=2, **kwargs):
    model = VAN_CCNet(num_classes=num_classes, recurrence=recurrence)

    return model


if __name__ == "__main__":
    model = VAN_CCNet(num_classes=151, recurrence=2)
